...
export function generateAiQuestion({
  ...
  previousQuestions: Pick<Question, "content">[]; // Prisma Question has `content`
  difficulty: ExperienceLevel;
  onFinish: (question: string) => void;
}) {
  const previousMessages: CoreMessage[] = previousQuestions.flatMap((q) => [
    { role: "user", content: difficulty }, 
    { role: "assistant", content: q.content },
  ]);
...
- A difficulty level of "junior", "mid_level", or "senior" is provided by the user (from Prisma enum) and should be used to tailor the question.
...
- It is ok to ask a question about just a single part of the job description, such as a specific technology or skill (e.g., if the job description is for a Next.js, Prisma, and TypeScript developer, you can ask a TypeScript-only question).
